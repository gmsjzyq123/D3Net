...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    """ 获取指定名称的数据集并返回相关组件。
       参数：
           dataset_name：数据集的名称字符串
           target_folder（可选）：存储数据集的文件夹，默认为当前目录
           datasets（可选）：数据集配置字典，默认使用预定义的配置
       返回：
           img：3D高光谱图像（WxHxB）
           gt：2D标签的整数数组
           label_values：类别名称的列表
           ignored_labels：要忽略的整数类别的列表
           rgb_bands：对应红、绿和蓝波段的整数元组
    """
    palette = None
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.flip_augmentation and self.patch_size > 1 and np.random.random() < 0.5:  # "<0.5?"
